...
export function GamificationManager({ config, onChange, onSave, saving, message }: { config: GamificationConfig; onChange: (value: GamificationConfig) => void; onSave: () => void; saving: boolean; message: string }) {
  ...
  return <section className="admin-gamification gamification-manager">
    ...
    {tab === 'levels' && <div className="gamification-config-grid">{config.levelConfig.map((level, index) => <div className="gamification-config-card" key={index}><div><b>{level.name}</b><small>{level.detail}</small></div><div className="gamification-inline-fields"><input value={level.name} onChange={(event) => { const next = [...config.levelConfig]; next[index] = { ...level, name: event.target.value }; onChange({ ...config, levelConfig: next }) }} /><input value={level.detail} onChange={(event) => { const next = [...config.levelConfig]; next[index] = { ...level, detail: event.target.value }; onChange({ ...config, levelConfig: next }) }} /><input type="number" value={level.target} onChange={(event) => { const next = [...config.levelConfig]; next[index] = { ...level, target: Number(event.target.value) }; onChange({ ...config, levelConfig: next }) }} /></div></div>)}</div>}
    ...
    {(tab === 'levels' || tab === 'rewards') && <button className="gamification-save-button" onClick={onSave} disabled={saving}>{saving ? 'SALVANDO…' : 'SALVAR CONFIGURAÇÃO'}</button>}
  </section>
}
