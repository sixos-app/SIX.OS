...
export const onRequestPost: PagesFunction<LibraryBindings, 'id'> = async ({ env, params, request }) => {
  ...
  const folder = await env.DB.prepare('SELECT id, slug FROM project_library_folders WHERE id = ? AND project_id = ? LIMIT 1')
    .bind(folderId, project.id)
    .first<FolderRow>()
  if (!folder) return Response.json({ error: 'Pasta não encontrada neste projeto' }, { status: 404 })
  ...
  const existingFile = await env.DB.prepare(`
    ...
    LIMIT 1
  `).bind(project.id, folder.id, name).first<ExistingFileRow>()
  ...
  await env.FILES.put(storageKey, file.stream(), {
    ...
    customMetadata: { projectId: project.id, folderId: folder.id, fileId, version: String(version) },
  })

  try {
    const statements = [
      existingFile
        ? env.DB.prepare(`UPDATE project_library_files SET file_type = ?, size_bytes = ?, storage_provider = 'r2', storage_key = ?, version = ?, created_by_user_id = ?, updated_at = ? WHERE id = ?`)
          .bind(fileTypeFrom(file), file.size, storageKey, version, user.id, now, fileId)
        : env.DB.prepare(`INSERT INTO project_library_files (id, project_id, folder_id, name, file_type, size_bytes, storage_provider, storage_key, version, created_by_user_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'r2', ?, ?, ?, ?, ?)`)
          .bind(fileId, project.id, folder.id, name, fileTypeFrom(file), file.size, storageKey, version, user.id, now, now),
      env.DB.prepare(`INSERT INTO project_library_file_versions (id, file_id, version, storage_key, size_bytes, created_by_user_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`)
