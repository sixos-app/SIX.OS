...
export const onRequestPost: PagesFunction<Bindings, { id: string }> = async ({ env, params, request }) => {
  ...
  const mission = await env.DB.prepare(`
    SELECT missions.id, missions.xp_reward, missions.ideas_reward, missions.status
    FROM missions
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`UPDATE gamification_profiles SET xp = xp + ?, ideas = ideas + ?, updated_at = ? WHERE user_id = ?`).bind(mission.xp_reward, mission.ideas_reward, completedAt, user.id),
    env.DB.prepare(`INSERT INTO xp_events (id, user_id, mission_id, xp, ideas, event_type, created_at) VALUES (?, ?, ?, ?, ?, 'mission_completed', ?)`)
      .bind(crypto.randomUUID(), user.id, mission.id, mission.xp_reward, mission.ideas_reward, completedAt),
  ])
