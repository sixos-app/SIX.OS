...
export const onRequestPost: PagesFunction<Bindings> = async ({ env, request }) => {
  ...
  const workflowDepartments = Array.isArray(body?.workflowDepartments)
    ? body.workflowDepartments.filter((value): value is string => typeof value === 'string').map((value) => value.trim().slice(0, 80)).filter(Boolean).slice(0, 8)
    : []
  const workflowSteps = Array.isArray(body?.workflowSteps)
    ? body.workflowSteps.map((value) => {
        if (!value || typeof value !== 'object') return null
        const step = value as { departmentName?: unknown; responsibleUserId?: unknown }
        ...
        return departmentName && responsibleUserId ? { departmentName, responsibleUserId } : null
      }).filter((value): value is { departmentName: string; responsibleUserId: string } => Boolean(value)).slice(0, 8)
    : workflowDepartments.map((departmentName) => ({ departmentName, responsibleUserId: assigneeId }))
  ...
  await env.DB.batch([
    env.DB.prepare(`INSERT INTO missions (id, project_id, client_id, title, description, status, priority, xp_reward, reward_label, xp_rule_id, xp_recipient_user_id, current_workflow_position, board_id, stage_id, due_at, created_by_user_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'open', ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?)`)
      .bind(missionId, project.id, project.clientId, title, description, priority, xpRule?.baseXp ?? xpReward, rewardLabel, xpRule?.id ?? null, assignee.id, workflow.board.id, initialStage.id, dueAt, user.id, now, now),
  ...
  if (workflowSteps.length) {
    await env.DB.batch(workflowSteps.map((step, position) => env.DB.prepare('INSERT INTO mission_workflow_steps (id, mission_id, position, department_name, responsible_user_id, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), missionId, position, step.departmentName, step.responsibleUserId, position === 0 ? 'active' : 'pending', now)))
  }
  return Response.json({ id: missionId, title, projectId: project.id, assigneeId: assignee.id, boardId: workflow.board.id, stageId: initialStage.id, stageName: initialStage.name, stageType: initialStage.type, currentDepartment: workflowSteps[0]?.departmentName ?? null, nextDepartment: workflowSteps[1]?.departmentName ?? null, currentResponsibleUserId: workflowSteps[0]?.responsibleUserId ?? null, nextResponsibleUserId: workflowSteps[1]?.responsibleUserId ?? null, dueAt, priority, xpReward: xpRule?.baseXp ?? xpReward, xpRuleId: xpRule?.id ?? null, description }, { status: 201 })
}
