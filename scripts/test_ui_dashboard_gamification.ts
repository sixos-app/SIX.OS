...
assert.equal(homeBaseContract.match(/\.momentum-art\s*\{/g)?.length, 1)
assert.equal(mobileFlowContract.match(/\.momentum-art\s*\{/g)?.length, 1)
...
assert.match(homeBaseContract, /\.momentum-decoration \{[^}]*position: relative;[^}]*grid-column: 2;[^}]*grid-row: 1 \/ -1;[^}]*pointer-events: none;/)
assert.match(homeBaseContract, /\.momentum-decoration-visual \{[^}]*align-self: stretch;[^}]*justify-self: stretch;/)
assert.match(homeBaseContract, /\.momentum-art \{[^}]*position: relative;[^}]*grid-column: 2;[^}]*grid-row: 1 \/ -1;[^}]*background: transparent;[^}]*border: 0;[^}]*box-shadow: none;[^}]*outline: 0;[^}]*overflow: visible;/)
...
assert.match(mobileFlowContract, /\.momentum-decoration, \.momentum-art \{ grid-area: badge; \}/)
assert.match(mobileFlowContract, /\.momentum-art \.level-badge \{ --level-badge-size: clamp\(64px, 22vw, 98px\); \}/)
assert.doesNotMatch(mobileFlowContract, /\.momentum-badge\s*\{|\.xp-meter \{[^}]*position:/)
